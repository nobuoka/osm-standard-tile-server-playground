...
task_definition_name=$($terraform output "${env}_task_definition_util")
...
# Run task
task_arn=$($aws --profile osm-tile ecs run-task --cluster $cluster_name --task-definition $task_definition_name \
...
$aws --profile osm-tile ecs wait tasks-stopped --cluster $cluster_name --tasks $task_arn

task_exit_code=$($aws --profile osm-tile ecs describe-tasks --cluster $cluster_name --tasks $task_arn \
